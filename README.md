...
These elements are described in different folders. All the modelling happens within the `openfisca_nsw_fisheries` folder.
...
```sh
pip install openfisca_nsw_fisheries
```
...
```sh
git clone https://github.com/openfisca/openfisca-nsw_fisheries.git
cd openfisca-nsw_fisheries
...
Substitute your package's country name for `openfisca_nsw_fisheries` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_nsw_fisheries/situation_examples/couple.json \
